...
import { LogOut, UserCog, Eye, EyeOff, ChevronDown, BarChart3, Users, Database, LayoutDashboard, CreditCard, Menu, X, Home, Wallet, ArrowLeftRight, ShieldAlert } from 'lucide-react'
...
const TABS_ADMIN = [
  ...
  { label: 'Recuperaciones', to: '/admin/recuperaciones', match: ['/admin/recuperaciones'], icon: ShieldAlert },
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {!sidebarCollapsed && (
              <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                {isAdmin && (
                  <select
                    value={localStorage.getItem('hb_admin_role') || 'ASESOR'}
                    onChange={(e) => {
                      localStorage.setItem('hb_admin_role', e.target.value);
                  ...
                  >
                    <option value="ASESOR">Asesor de Negocios</option>
                    <option value="JEFE_REGIONAL">Jefe Regional</option>
                    <option value="COMITE">Comité de Riesgos</option>
                  </select>
                )}
              </div>
            )}
